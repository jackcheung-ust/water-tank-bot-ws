...
class SerialComs {
    ...
    private:
        ...
        int send_heartbeat(uint8_t packet_id) {
            ...
            // Pack the data
            uint8_t packed_data[sizeof(heartbeat_data)+6];
            // uint8_t packed_data[20];  // Buffer for packed data (sufficient for worst case)
            uint8_t packed_size = pack_data(packed_data, heartbeat_data, 5);
        ...
        int send_auto_ack(uint8_t packet_id){
            ...
            uint8_t packed_data[sizeof(ack_data)+6];
            // uint8_t packed_data[20];  // Buffer for packed data (sufficient for worst case)
            uint8_t packed_data_size = pack_data(packed_data, ack_data, sizeof(ack_data));
        ...
        int send_motor_control(uint8_t packet_id,int16_t l_motor_v, int16_t r_motor_v){
            ...
            uint8_t packed_data[sizeof(control_data)+6];

            uint8_t packed_data_size = pack_data(packed_data, control_data, sizeof(control_data));
        ...
        void parsing_data(LibSerial::DataBuffer &dataBuffer){
            ...
            uint8_t unpacked_data[50];
            
            
            int unpacked_size = unpack_data(dataBuffer.data(), unpacked_data, 30);

            if (unpacked_data[3]==2 && unpacked_data[4]==0){
            ...
            else if (unpacked_data[3] == 1 && unpacked_data[4]==1){
                operate_mode_ = unpacked_data[7];
            ...
            else if (unpacked_data[3]==1 && unpacked_data[4]==2){
                operate_mode_ = unpacked_data[7];
                left_w_speed_feedback_ = unpacked_data[8] << 8 | unpacked_data[9];
                right_w_speed_feedback_ = unpacked_data[10] << 8 | unpacked_data[11];
                roll_ = unpacked_data[12] << 8 | unpacked_data[13];
            ...
            else if (unpacked_data[3]==1 && unpacked_data[4]==0){
                operate_mode_ = unpacked_data[7];
                left_w_speed_feedback_ = unpacked_data[8] << 8 | unpacked_data[9];
                right_w_speed_feedback_ = unpacked_data[10] << 8 | unpacked_data[11];
                // roll_ = unpacked_data[12] << 8 | unpacked_data[13];
